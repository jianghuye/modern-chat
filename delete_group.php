...
try {
    ...
    if ($group_info['owner_id'] != $user_id) {
        echo json_encode(['success' => false, 'message' => '只有群主可以解散群聊']);
        exit;
    }

    // 解散群聊
    $result = $group->deleteGroup($group_id, $user_id);
    if ($result) {
        echo json_encode(['success' => true, 'message' => '群聊已成功解散']);
    } else {
        echo json_encode(['success' => false, 'message' => '解散群聊失败']);
    }
} catch (Exception $e) {
    // 捕获所有异常并返回错误信息
